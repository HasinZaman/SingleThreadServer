...
impl Method {
    pub fn new(method : String, target : String, body : Option<Body>) -> Result<Method, ParserError> {
        ...
        match method.as_str() {
            ...
            "POST"      => return Result::Ok(Method::POST{ file : target.to_string(), body : body_unwrap(body)?}),
            "PUT"       => return Result::Ok(Method::PUT{ file : target.to_string(), body : body_unwrap(body)?}),
            "DELETE"    => return Result::Ok(Method::DELETE{ file : target.to_string(), body : body}),
            ...
            "PATCH"     => return Result::Ok(Method::PATCH{ file : target.to_string(), body : body_unwrap(body)?}),
